...
export default function ExpandingLottieWindow() {
    ...
    const pageObserver = () => {
        ...
        useEffect(() => {
            if (inView) {
                // Fetch data when the element comes into view
                fetchMoreItems();
            }
        }, [inView]); // Reruns effect when 'inView' status changes

        return (
            <div>
                {/* List of items */}
                <div ref={ref}>
                    {/* Invisible target element to observe */}
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="w-[100vw] h-[300vh] flex flex-col relative" ref={sectionRef}>
            ...
            <section ref={section2Ref} className="relative w-[100vw] h-[100vh] z-20">
            </section>
